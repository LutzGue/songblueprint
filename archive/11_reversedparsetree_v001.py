...
# Konvertiere den String in eine verschachtelte Liste
...
# Beispiel Parsetree als String
parsetree_str = "[A[B[D x1]][C[E[G x2][H[I x3]]][F x4]]]"

# Konvertiere den String in eine nested List und entferne Leerzeichen
parsed_tree = parse_string(parsetree_str.replace(" ", ""))
...
"""
# version: multiple characters, issue last node without number suffix
def parse_tree(tree, root=None, path=None):
    if path is None:
        path = []
    if root is None:
...
def parse_string(input_str):
    ...
    while i < len(input_str):
        ...
        if char.isalpha() or char.isspace():
            if char.isalpha() or (char.isspace() and current_list and current_list[-1] and isinstance(current_list[-1], str)):
                if current_list and current_list[-1] and isinstance(current_list[-1], str):
                    current_list[-1] += char
                else:
                    current_list.append(char)
            elif char.isspace() and not current_list:
                # Wenn Leerzeichen am Anfang vorhanden sind
                current_list.append(char)
            else:
                current_list.append(char)
        elif char == '[':
...
def parse_string(input_str):
    ...
    while i < len(input_str):
        ...
        if char.isalpha() or char.isspace():
            if char.isalpha() or (char.isspace() and current_list and current_list[-1] and isinstance(current_list[-1], str)):
                if current_list and current_list[-1]:
                    current_list[-1] = current_list[-1] + char if isinstance(current_list[-1], str) else char
                else:
                    current_list.append(char)
            elif char.isspace() and not current_list:
                # Wenn Leerzeichen am Anfang vorhanden sind
...
def parse_string(input_str):
    ...
    while i < len(input_str):
        ...
        if char.isalpha() or char.isspace():
            if char.isalpha() or (char.isspace() and current_list and current_list[-1] and isinstance(current_list[-1], str)):
                if current_list and current_list[-1] and isinstance(current_list[-1], str) and (not current_list[-1] or current_list[-1][-1].isalpha()):
                    current_list[-1] += char
                else:
                    current_list.append(char)
            elif char.isspace() and not current_list:
